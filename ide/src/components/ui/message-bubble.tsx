...
import { NvidiaLogo } from "@/components/ui/message-bubble-utils/nvidia";
import { QwenLogo } from "@/components/ui/message-bubble-utils/qwen";
import { ZaiLogo } from "@/components/ui/message-bubble-utils/zai";

type MessageRole = "user" | "assistant";

type Provider = "google" | "qwen" | "zai" | "nvidia";
...
export function AiMessageBubble({
  ...
  const defaultAvatar = isUser ? (
    <div className="flex size-7 items-center justify-center rounded-[var(--radius-sm)] border bg-foreground text-background">
      <User className="size-3.5" />
    </div>
  ) : ProviderLogo ? (
    <div className="flex size-7 items-center justify-center rounded-[var(--radius-sm)] border bg-background">
    ...
    >
      ...
      >
        <div aria-live={isStreaming ? "polite" : undefined}>
          ...
          {isStreaming && (
            <span className="ai-caret ml-1 inline-block h-3.5 w-[2px] bg-current" />
          )}
        </div>
