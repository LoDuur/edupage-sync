...
import { DEFAULT_MODEL, modelOf } from "./lib/models.js";
...
export const useStore = create((set, get) => ({
  ...
  tabs: [], activeTab: null,
  view: "workspace", sidebarOpen: ls("sidebar", "1") === "1", aiOpen: ls("ai-open", "0") === "1", aiWidth: Math.max(280, +ls("ai-w", 380) || 380),
  dock: { view: "terminal", open: true, max: false },
  run: { state: "idle", rc: null, ms: 0, provider: null, phase: "" },
  problems: [], output: "", server: null,
  executor: ls("executor", "auto"),
  model: modelOf(ls("java-ai-model", "")) ? ls("java-ai-model", "") : DEFAULT_MODEL,
  fontSize: +ls("code-font", 13), wrap: ls("wrap", "0") === "1", minimap: ls("minimap", "0") === "1",
  indents: readJson("indents", {}),
  savedAt: 0, palette: false,
  toast: null,
  set,
  ...
  toggleAI: v => set(s => { const aiOpen = v === undefined ? !s.aiOpen : v; localStorage.setItem("ai-open", aiOpen ? "1" : "0"); return { aiOpen, sidebarOpen: aiOpen && innerWidth < 1024 ? false : s.sidebarOpen }; }),
  setAiWidth: w => { const aiWidth = Math.round(Math.min(Math.max(280, w), innerWidth * 0.6)); localStorage.setItem("ai-w", aiWidth); set({ aiWidth }); },
  setDock: d => set(s => ({ dock: { ...s.dock, ...d } })),
  setRun: r => set(s => ({ run: { ...s.run, ...r } })),
  setProblems: problems => set({ problems }),
  setOutput: output => set({ output }),
  appendOutput: t => set(s => ({ output: (s.output + t).slice(-200000) })),
  setExecutor: executor => { localStorage.setItem("executor", executor); set({ executor }); },
  setModel: model => { localStorage.setItem("java-ai-model", model); set({ model }); },
  ...
  setIndent: (lang, ind) => set(s => { const indents = { ...s.indents, [lang]: ind }; localStorage.setItem("indents", JSON.stringify(indents)); return { indents }; }),
