/* shared: access gate, toast, helpers */
const UI = (() => {
  const esc = s => String(s).replace(/[&<>"]/g, c => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;" }[c]));
  const $ = id => document.getElementById(id);
  const ico = (name, cls = "") => `<i class="codicon codicon-${name} ${cls}" aria-hidden="true"></i>`;
  ...
  async function requireAccess(onOk, { sub = "Enter the class access code", back = "../" } = {}) {
    const k = accessKey();
    ...
    g.innerHTML = `<div class="card"><h1>28teh</h1><p class="sub">${esc(sub)}</p>
      <div class="field">${ico("lock")}<input id="gate-in" autocomplete="off" autocapitalize="characters" spellcheck="false" placeholder="Access code"></div>
      <div class="err" id="gate-err"></div>
      <div class="row"><button class="btn pri" id="gate-ok">Continue</button><a href="${back}">${ico("arrow-left")} Timetable</a></div></div>`;
    ...
    async function tryIt() {
      ...
      else { err.textContent = "Incorrect access code"; card.classList.remove("shake"); void card.offsetWidth; card.classList.add("shake"); inp.select(); }
    }
    g.querySelector("#gate-ok").onclick = tryIt;
    inp.addEventListener("keydown", e => { if (e.key === "Enter") tryIt(); });
  }

  function toast(t, kind = "info") {
    let el = $("toast");
    if (!el) {
    ...
    el.innerHTML = ico(kind === "error" ? "error" : kind === "ok" ? "check" : "info") + esc(t); el.classList.add("on"); toast._at = performance.now();
    clearTimeout(toast._t); toast._t = setTimeout(() => el.classList.remove("on"), 2500);
  }

  return { esc, $, ico, requireAccess, accessKey, sha256, toast };
})();
