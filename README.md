...
## 28teh – compiler (`compiler/`)

`compiler/` ("28teh", console command `compiler`, `java`, `pil`) is a VS Code-style IDE (English UI): activity bar, explorer/saved/live sidebar, editor tabs, **Monaco** editor with a Dark Modern theme and per-language completion, a bottom panel with Problems (Monaco markers), Output (raw compiler text) and an **xterm.js** terminal where program stdin is typed inline, AI Chat as a secondary side bar, status bar. Codicons for icons, Inter + JetBrains Mono. Files: `compiler/index.html`, `ide.css`, `ide.js`, shared `ui.css`/`ui.js`; `compiler/works/` redirects into the IDE's Saved view. Languages: Java (Wandbox OpenJDK 22), Python, C, C++, C# (mono), Lua (all Wandbox) and PIL (WASM). Per-language config lives in `compiler/langs.js` (templates, examples, completion keywords, Wandbox compiler id). The terminal pane is both output and stdin: when a program asks for input it pauses, the prompt turns amber, and each line typed there resumes the program (both engines are batch, so the page re-runs deterministically with the accumulated stdin and shows only the new output; Java detects the pause via `NoSuchElementException`, Python via `EOFError`, C/C++ via a `-include shim.h` that wraps `std::cin`'s streambuf and the stdio read macros and exits with a marker on EOF, Lua by wrapping `io.read`, C# by wrapping `Console.In` in a class appended after the user's code; PIL via the WASM stdin callback. Ctrl+D sends a real EOF, Ctrl+C aborts). Errors are parsed per language into a clickable list (line, message, short Latvian hint), highlighted in the editor with inline messages; exit codes 137/139/134/136 and Wandbox timeouts/rate limits get readable messages. **Live Coding** (`live.js`): the host starts a session (class key required, optional viewer password) stored in `live_sessions`; code is pushed via `live_update` (throttled) and viewers get it through Supabase Realtime (`postgres_changes`, polling fallback). With a password the code is AES-GCM encrypted client-side (PBKDF2 from the password, session id as salt), so the server never sees plaintext. Active sessions appear at the top of `compiler/works/` with a language badge; a session ends on "Beigt", page close, or 90 s without heartbeat. Java code is compiled with [Wandbox](https://wandbox.org) (OpenJDK 22, UTF-8). Saving goes to Supabase (project `edupage-sync`, table `snippets`) through the `save_snippet` RPC, which checks a bcrypt-hashed class passkey stored in `settings`. Anonymous clients can only read: saved snippets are public and immutable (no insert/update/delete grants). The same code gates the page itself (`compiler/`) and the works archive (`compiler/works/`) client-side (SHA-256 in `compiler/ui.js`), and is checked server-side for saving. Change it with `select set_passkey('new-key');` in the Supabase SQL editor and update `ACCESS_HASH` in `compiler/ui.js`. The editor has autocompletion (Tab / Ctrl+Space, snippets), error-line highlighting and an AI helper whose instructions live in `compiler/prompts.js` (teacher role, per-language rules, quick actions); models come from the Cloudflare Worker (`/ai/*`).

## PIL interpreter (`compiler/pil/`)

PIL is the second language in the same editor (`compiler/?lang=pil`; `compiler/pil/` redirects there): [PIL](https://github.com/Acerx-AMJ/PIL), a small COBOL/assembly-style esolang. The upstream C++ interpreter is compiled to WebAssembly (`compiler/pil/pil.js`, single file) and runs in a Web Worker inside the browser: no server, stdin from the `stdin` tab, 10 s timeout per run, Latvian UTF-8 output. Rebuild with `tools/build-pil.sh` (needs `brew install emscripten`; built with `-fwasm-exceptions` because `to-int`/`to-float` rely on C++ exceptions); the interpreter commit is in `compiler/pil/VERSION`. `compiler/pil/pil-lang.js` holds the CodeMirror mode, Tab completion (all built-ins with argument hints, snippets `main func loop if readnum …`), bundled examples and the language reference shown in the `Valoda` tab and given to the AI helper. Note the upstream README is older than the code: the real names are `println printfln readln readch const` (the page hints this on `No such function` errors). Saved works share the `snippets` table with Java through the `lang` column (`save_snippet(..., p_lang)`); the works archive shows both with a language filter.
